...
Real-time visualization of Claude Code, Codex, and Antigravity agent orchestration. Watch your agents think, branch, and coordinate as they work. [Demo video here](https://www.youtube.com/watch?v=Ud6eDrFN-TA). 
...
- **Claude Code + Codex + Antigravity**: Auto-detects sessions from all three runtimes concurrently and shows them side-by-side, or restrict to one via the `agentVisualizer.runtime` setting
...
- **Antigravity integration**: Directly tails Antigravity CLI's logs `~/.gemini/antigravity-cli/brain/**/transcript.jsonl` to provide realtime simulation
...
3. Start a Claude Code, Codex, or Antigravity session in your workspace — Agent Flow will auto-detect it
...
By default Agent Flow watches Claude Code (`~/.claude/projects/`), Codex (`~/.codex/sessions/`), and Antigravity (`~/.gemini/antigravity-cli/brain/`) concurrently in all three entry points (VS Code extension, `pnpm run dev`, `npx agent-flow-app`). Sessions are shown side-by-side and tagged by runtime. If you only use one, the others are harmless no-ops — no visible effect, no user action needed.

To restrict to one runtime:

- **VS Code extension:** set `agentVisualizer.runtime` to `"auto"` / `"claude"` / `"codex"` / `"antigravity"` in your settings
- **`pnpm run dev` and `npx agent-flow-app`:** set the `AGENT_FLOW_RUNTIME` environment variable to `claude`, `codex`, or `antigravity` (defaults to watching all)
...
|---------|---------|-------------|
| `agentVisualizer.runtime` | `"auto"` | Which agent runtime(s) to watch: `"auto"` (all), `"claude"`, `"codex"`, or `"antigravity"` |
